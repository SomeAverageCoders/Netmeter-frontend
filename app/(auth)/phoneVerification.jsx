...
const PhoneVerification = () => {
  ...
  const confirmVerificationCode = async () => {
    try {
      ...
      if (response.data && response.data.statusCode === 201) {
          ...
          router.push({
            pathname: "/(auth)/unsuccess",
            params: { message: "Incorrect OTP. Please enter the correct code." }
          });
      }
    } catch (error) {
          router.push({
            pathname: "/(auth)/unsuccess",
            params: { message: "Oops, an error occurred. Please try again." }
          });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResendOTP = () => {
    // Here you would call your backend API to resend the OTP
    console.log("Resend OTP requested for:", signupData?.phone, signupData?.email);
    setMessage("A new OTP has been requested. Please check your phone.");
  };
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity onPress={handleResendOTP} disabled={isLoading}>
        <Text className="text-blue-500 underline text-center mt-4">
